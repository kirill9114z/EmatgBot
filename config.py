...
def load_wae_change(prefix: str):
    ...
    ВАЖНО, не опечатка: при CHANGE_X_TIMEFRAIM='1d' берётся ОДНА свеча, и
    изменение получается ровно 0 — старый код делал fetch_ohlcv(limit=1) и
    сравнивал close свечи с её же close. Фильтр с порогом 0 при этом
    пропускает всё, а в сообщении печатается '1d 0%'.

    Выглядит как ошибка оригинала, но воспроизводится намеренно: на этом
    держится работа чатов, у которых период задан как '1d'. Если поставить
    минимум в 2 свечи, фильтр становится настоящим и у SELL-сигналов вступает
    в противоречие с требованием неотрицательного изменения — чат замолкает
    полностью (проверено на истории: 0 сигналов из 41 прошедшего WAE).
    Нужен реальный суточный фильтр — пишите '2d'.
    """
    ...
    return {
        "ENABLED": enabled,
        # Сколько дневных свечей берём. Число идёт как есть, включая 1 —
        # см. пояснение выше. Выключённый фильтр всё равно показывает
        # изменение за сутки, как в старой версии.
        "CANDLES": max(1, candles) if enabled else 2,
        "MIN_PCT": min_pct,
        "LABEL": label,
    }

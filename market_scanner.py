...
Обслуживает две независимые группы чатов в одном проходе:
    * CHAT_J / CHAT_K — фильтры по свечам (цвет, размер тела, RSI);
...
async def scan_once(exchange, chats, scan_cfg, out_queue, wae_chats=None, wae_cfg=None):
    ...
    Свечные чаты (CHAT_J/B) и WAE-чаты обслуживаются в одном проходе намеренно:
    ...
    # обеих групп: если CHAT_G и CHAT_J сидят на одном таймфрейме, запросы к
